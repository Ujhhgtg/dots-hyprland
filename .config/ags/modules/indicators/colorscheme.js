          ...
          ConfigToggle({
            ...
            onChange: (self, newValue) => {
              ...
              execAsync([
                `bash`,
                `-c`,
                `mkdir -p ${GLib.get_user_state_dir()}/dots-hyprland/user && sed -i "2s/.*/${transparency}/"  ${GLib.get_user_state_dir()}/dots-hyprland/user/colormode.txt`,
              ])
                .then(
                  execAsync([
                    "bash",
                    "-c",
                    `~/.local/bin/dots-hyprland/color_generation/switchcolor.sh`,
                  ]),
                )
                .catch(print);
            },
                ...
                onChange: (value, name) => {
                  ...
                  if (value)
                    execAsync([
                      `bash`,
                      `-c`,
                      `~/.local/bin/dots-hyprland/color_generation/switchcolor.sh - --yes-gradience`,
                      `&`,
                    ]).catch(print);
                  else
                    execAsync([
                      `bash`,
                      `-c`,
                      `~/.local/bin/dots-hyprland/color_generation/switchcolor.sh "${ADWAITA_BLUE}" --no-gradience`,
                      `&`,
                    ]).catch(print);
                },
              }),
          ...
          ConfigMulipleSelection({
            ...
            onChange: (value, name) => {
              execAsync([
                `bash`,
                `-c`,
                `mkdir -p ${GLib.get_user_state_dir()}/dots-hyprland/user && sed -i "3s/.*/${value}/" ${GLib.get_user_state_dir()}/dots-hyprland/user/colormode.txt`,
              ])
                .then(
                  execAsync([
                    "bash",
